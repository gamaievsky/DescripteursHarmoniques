...
en affichant en dessous les valeurs
'''
              ...
              if (i<j):
                  for k1 in range(1,self.K + 1):
                      for k2 in range(1,self.K + 1):
                          fmin = min(k1*self.frequenceAvecTemperament(pitch2), k2*self.frequenceAvecTemperament(pitch1))
                          fmax = max(k1*self.frequenceAvecTemperament(pitch2), k2*self.frequenceAvecTemperament(pitch1))
                          s=0.24/(0.021*fmin+19.)
                          self.dissonance = self.dissonance + (100/(k1*k2) ** self.decr) * (np.exp(-3.5*s*(fmax-fmin))-np.exp(-5.75*s*(fmax-fmin)))
      n = self.nombreDeNotes
...
score = converter.parse('/Users/manuel/Github/DescripteursHarmoniques/ExemplesMusicaux/AccordsMineur.musicxml')
