...
'''
Prend un contrepoint à trois voix, enlève la voix du milieu pour la remplacer par la voix obtenue par maximisation
de la concordance
'''
...
class ListeAccords:
          ...
          if gH.verticality.bassTimespan != None :
              ...
              if element.isNote or element.isChord:
                  ...
                  if 'concordance' in resultList:
                      ...
                      dataString = dataString + str(round(gH.concordance,1))
  ...
  def TroisiemeVoix(self):
      ...
      for verticality in self.tree.iterateVerticalities():
          ch = verticality.bassTimespan.element

          p = ch[0].transpose(1)
          conc = 0
          i = 1
          while ch[0].transpose(i+1).pitch < ch[2].pitch:
              val = np.sum(self.spectre(self.frequenceAvecTemperament(ch[0].pitch))*self.spectre(self.frequenceAvecTemperament(ch[0].transpose(i).pitch)))
              val = val + np.sum(self.spectre(self.frequenceAvecTemperament(ch[2].pitch))*self.spectre(self.frequenceAvecTemperament(ch[0].transpose(i).pitch)))
              val = val + np.sum(self.spectre(self.frequenceAvecTemperament(ch[0].pitch))*self.spectre(self.frequenceAvecTemperament(ch[2].pitch)))
              if  val > conc:
                  conc = val
                  p = ch[0].transpose(i)
              i = i+1
          s.append(chord.Chord([ch[0], p, ch[2]]))
      s.show()


      #vert1 = self.tree.getVerticalityAt(1.0)
      #for verticality in self.tree.iterateVerticalities():
          #print(verticality.startTimespans.element.pitches[1])
          #print(self.tree.getVerticalityAt(verticality.offset))
...
score = converter.parse('/Users/manuel/Dropbox (TMG)/Thèse/Disposition/Pal.musicxml')
l = ListeAccords(score)
l.ConcordanceCoherenceConcordanceOrdre3Liste()
#l.getAnnotatedStream(['concordance'])
#l.grandeursHarmoniques
l.TroisiemeVoix()
